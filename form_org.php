 ...
 * XHTML対応（XML宣言の処理）
 *
 * 2011-2013 (c) Crytus corporation.
 */
...
foreach ($form_input as $val) {
	$value = 0;
	if ($val["func"] == 6) {
		$form[$val["name"]] = $_REQUEST[$val["name"]];
		if ($form[$val["name"]]) {
		...
		$form[$val["name"] . "_pref"] = $_REQUEST[$val["name"] . "_pref"];
		$form[$val["name"] . "_address"] = $_REQUEST[$val["name"] . "_address"];
		if ($form[$val["name"] . "_pref"] && $form[$val["name"] . "_address"]) {
			$value = 1;
		}
	} else if ($val["func"] == 11) {
		$form[$val["name"] . "_year"] = $_REQUEST[$val["name"] . "_year"];
		$form[$val["name"] . "_month"] = $_REQUEST[$val["name"] . "_month"];
		$form[$val["name"] . "_day"] = $_REQUEST[$val["name"] . "_day"];
